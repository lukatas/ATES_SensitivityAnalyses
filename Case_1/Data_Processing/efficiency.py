...
for d in directories:
    ...
    for month in range(len(months_ends) - 1):
        if month == 0:
            ...
            case.loc[
                (case.index > months_ends[month])
                & (case.index <= months_ends[month + 1]),
                "month",
            ] = month + 1
        else:
            case.loc[
    ...
    for season in range(len(season_ends) - 1):
        if season == 0:
            ...
            case.loc[
                (case.index >= season_ends[season])
                & (case.index <= season_ends[season + 1]),
                "Seasons",
            ] = season + 1
        else:
            case.loc[
    ...
    summer_months = [0, 1, 2, 3, 8, 9, 10, 11, 16, 17, 18, 19]

    case["DeltaT_inj"] = np.where(
        case.month.isin(summer_months), case.deltaT_warm, case.deltaT_cold
    )
    case["DeltaT_extr"] = np.where(
        case.month.isin(summer_months), case.deltaT_cold, case.deltaT_warm
    )
    ...
    case["P_extr"] = (
        np.array(Q)[case["month"].astype(int).values] * 1.16 * case["DeltaT_extr"]
    )
    case["P_inj"] = (
        np.array(Q)[case["month"].astype(int).values] * 1.16 * case["DeltaT_inj"]
    )
