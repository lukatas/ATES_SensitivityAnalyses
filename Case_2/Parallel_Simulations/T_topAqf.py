...
def T_TopAqf(ucn_file: str, cbc_file: str, layer_of_interest: int, gwT_natural):
    ...
    for i in range(len(cbb_times) - 1):
        if i == 0:
            ...
            indices_within_interval = np.where((ucn_times >= start_time) & (ucn_times < end_time))
    ...
    ucn_xr = xr.DataArray(ucn_data, coords={'time':ucn_times.tolist(), 'y': y, 'x': x,}, dims=['time','y', 'x'], name='Temperature (°C)')

    ucn_xr = ucn_xr.to_dataset()

    ucn_xr['Power'] = ucn_xr['Temperature (°C)'] * 0
    for i in range(len(mask)):
        ucn_xr['Power'][i] = (abs(ucn_xr['Temperature (°C)'][i] - gwT_natural)) * (cbb_data[mask[i]][layer_of_interest]*3600) * 1.16
    ...
    integral_power_by_season = (ucn_xr['Power'] * time_diff).groupby('season').sum(dim='time')
