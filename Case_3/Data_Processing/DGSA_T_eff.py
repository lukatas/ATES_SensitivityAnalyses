...
from pyDGSA.plot import plot_cdf, reformat_interactions, bubble_plot
...
fig, ax = vert_pareto_plot(cluster_sensitivity, np_plot=8, fmt='cluster_avg',
...
plt.show()
#%%
fig, ax = vert_pareto_plot(cluster_interact_sensitivity, np_plot=20,
                           fmt='cluster_avg',
                           colors=cluster_colors)
plt.tight_layout()
plt.show()
...
for m in range(models):
    ...
    ax.plot(head.index/(24*3600), head['{}'.format(m)]-6.77, label='{}'.format(m), linewidth = 0.5,color=color)
    ax.set(ylim=(-20, 20))
...
parameters['transmissivity_aqf_m2/d'] = parameters['aqf_dz']*parameters['Kh_aqf']*24*3600
...
transmissivity_grid = np.linspace(data_XY['transmissivity_aqf_m2/d'].min(),
                                   data_XY['transmissivity_aqf_m2/d'].max(), 100)
head_change_grid = np.linspace(data_XY['max_head_mTAW'].min(),
                                data_XY['max_head_mTAW'].max(), 100)
flowrate_grid = np.linspace(data_YZ['flowrate'].min(),
                              data_YZ['flowrate'].max(), 100)
...
data_combined = np.vstack([
    data_XY['transmissivity_aqf_m2/d'],
    data_XY['max_head_mTAW'],
    data_XZ['flowrate']
]).T
...
flowrate_value = data_YZ[(data_YZ['flowrate']>3.4) &
                               (data_YZ['flowrate']<3.5)].reset_index()['flowrate'][0]

slice_index = (np.abs(flowrate_grid - flowrate_value)).argmin()  # Get the index of the closest value
density_slice = density[:, :, slice_index]  # This will give you the density for the specific flow rate
...
plt.ylabel('Density')
plt.title(f'Flowrate = {flowrate_value.round(2)} m³/h, Transmissivity = {transmissivity_value.round()}')
...
contour = ax.contourf(transmissivity_grid, head_change_grid, density_slice.T, cmap='viridis')
ax.set_title(f"2D Slice at Flowrate = {flowrate_value.round(1)} m³/h")
ax.set_xlabel("Transmissivity (m²/d)")
ax.set_ylabel("Head Change (m)")
plt.savefig(os.path.join(Directories.fig_dir,'kde head vs trans fl 3.svg'),format='svg')
...
cumulative_density = np.cumsum(normalized_density) * np.diff(head_change_grid, prepend=head_change_grid[0])
...
if len(head_change_grid) != len(normalized_density):
    raise ValueError("The lengths of head_change_grid and normalized_density do not match!")
...
cumulative_probability = np.sum(normalized_density[:closest_index + 1] * interval_widths[:closest_index + 1])

print(f"Probability Density at Head Change {head_value_of_interest}: {probability_density:.4f}")
print(f"Cumulative Probability up to Head Change {head_value_of_interest}: {cumulative_probability:.4f}")
