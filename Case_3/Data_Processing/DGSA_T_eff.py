...
def calculate_energy(parameters, deltaT_warm, deltaT_cold, time_discretization, directories, skipped_indices):
    ...
    for idx, d in enumerate(directories):
        if idx in skipped_indices:
            ...
            case['Seasons'] = case.index // time_discretization - ((case.index%time_discretization==0) & (case.index != 0)).astype(int)
...
deltaT_warm, deltaT_cold, deltaT, parameters, directories, skipped_indices = load_and_process_data(directory, prefix, prefix2, time)
...
parameters['dispersion x advection velocity'] = ((parameters['gradient']/100 * parameters['Kh_aqf']) / parameters['por_Eaqf']) * parameters['longitudinal']
fig, ax = plt.subplots()
for m in range(new_size):
    x = (Eff_df.T[5][m])
