...
def calculate_energy(
    ...
    for idx, d in enumerate(directories):
        if idx in skipped_indices:
            ...
            )  # need to change to paramters[deltaT_inj][idx] when deltaT is not constant for all models
...
data = [
    unfiltered["longitudinal"],
    filtered_df["longitudinal"],
    filtered_df1["longitudinal"],
    filtered_df2["longitudinal"],
]
...
ax.set_xticks(
    [y + 1 for y in range(len(data))], labels=["prior", "good", "medium", "bad"]
)
...
#         edgecolor=color,
#         color="None",
#         label=f"Eff_df_80, Cluster {label}",
#     )
#     ax.bar(
#         x,
#         Eff_df_60[str(m)],
...
#         edgecolor=color,
#         color="None",
#         label=f"Eff_df_60, Cluster {label}",
#     )
#     ax.bar(
#         x + bar_width,
#         Eff_df_40[str(m)],
...
#         edgecolor=color,
#         color="None",
#         label=f"Eff_df_40, Cluster {label}",
#     )
#
# # Labeling
# ax.set_xlabel("Seasons (6 months)")
...
sns.kdeplot(
    data=parameters_0["por_Eaqf"], shade=True, bw_adjust=1.2, color=cluster_colors[0]
)
sns.kdeplot(
    data=parameters_1["por_Eaqf"], shade=True, bw_adjust=1.2, color=cluster_colors[1]
)
...
#         df_max_0[distance],
#         bar_width,
#         label=f"Cluster 0 - {distance}",
#         alpha=0.7,
#     )
#     ax.bar(
#         indices + (i * bar_width) + bar_width,
#         df_max_1[distance],
#         bar_width,
#         label=f"Cluster 1 - {distance}",
#         alpha=0.7,
#     )
#
